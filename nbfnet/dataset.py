...
@R.register("datasets.NELL995Inductive")
class NELL995Inductive(InductiveKnowledgeGraphDataset):
    ...
    def __init__(self, path, version="v1", verbose=1):
        ...
        for url in self.train_urls:
            url = url % version
            save_file = "NELL-995_%s_%s" % (version, os.path.basename(url))
            txt_file = os.path.join(path, save_file)
            if not os.path.exists(txt_file):
                txt_file = utils.download(url, self.path, save_file=save_file)
            train_files.append(txt_file)
        ...
        for url in self.test_urls:
            url = url % version
            save_file = "NELL-995_%s_ind_%s" % (version, os.path.basename(url))
            txt_file = os.path.join(path, save_file)
            if not os.path.exists(txt_file):
                txt_file = utils.download(url, self.path, save_file=save_file)
            test_files.append(txt_file)
